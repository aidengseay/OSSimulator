...
int getCommand(char *cmd, const char *inputStr, int index)
{
    ...
    while(index < lengthOfCommand)
    {
        ...
        // set next character to null charater
        cmd[index] = NULL_CHAR;
    }
                                                              ...
                                                              char *endStateMsg)
{
    ...
    *opCodeDataHead = NULL;
    ...
    // allocate memory for the temp data struct
    ...
    // check failure of first complete op command
    if(accessResult != COMPLETE_OPCMD_FOUND_MSG)
    {
        // close file
        fclose(fileAccessPtr);

        // clear data from the struct list
...
OpCodeMessages getOpCommand(FILE *filePtr, OpCodeType *inData)
{
    ...
    // get whole op command as a string, check for successful access
